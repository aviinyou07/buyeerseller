...
const Buyers = () => {
  ...
  const loadBuyers = async () => {
    try {
      ...
      const res = await api.get(`/customers?type=buyer&search=${encodeURIComponent(searchTerm)}&status=${statusQuery}&page=${currentPage}&limit=${itemsPerPage}`);
      ...
      if (res.success) {
        ...
        const mapped = items.map((item, idx) => {
          ...
          return {
            id: item.id,
            name: item.full_name || "Unnamed Buyer",
            handle: `@buyer_${item.id}`,
            initials,
            ...
            totalOrders: item.buyer_details?.total_orders_placed || 0,
            totalSpent: typeof item.buyer_details?.total_spent === 'number' ? `${Number(item.buyer_details.total_spent).toFixed(2)}` : (item.buyer_details?.total_spent || '$0.00'),
            raw: item,
          };
        });

        setBuyersData(mapped);
        ...
        if (summaryRes && summaryRes.success) {
          setMetrics({
            total: summaryRes.data.counters.totalBuyers,
            active: summaryRes.data.counters.totalBuyers - summaryRes.data.counters.blockedUsers, // Estimation or approximation
            pending: 0,
          ...
          setMetrics({
            ...
            rejected: mapped.filter(b => b.status === "Rejected").length
          });
        }
      }
    } catch (err) {
  ...
  const paginatedBuyers = buyersData;
  const totalPages = Math.ceil(totalItems / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + paginatedBuyers.length;
  ...
  const updateBuyerStatus = async (id, newStatus) => {
    try {
      ...
      alert(err.message || "Failed to update buyer status");
    }
  ...
  const deleteBuyer = async (id) => {
    try {
      if (window.confirm("Are you sure you want to delete this buyer account?")) {
        await api.delete(`/customers/${id}`);
        setBuyersData((prev) => prev.filter((buyer) => buyer.id !== id));
  ...
  if (selectedBuyer) {
    const isSeller = selectedBuyer.raw?.role === 'Seller' || selectedBuyer.raw?.role === 'Both' || selectedBuyer.raw?.role === 'seller';
    const hasAddress = !!selectedBuyer.raw?.address;
    ...
    return (
      <div className="space-y-6 w-full mx-auto animate-[fadeIn_.2s_ease] text-left p-4 lg:p-6 pb-20">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 lg:p-8 flex flex-col md:flex-row gap-8 items-start md:items-center">
          <div
            className={`w-24 h-24 sm:w-32 sm:h-32 rounded-full flex items-center justify-center text-4xl sm:text-5xl font-bold border-4 border-white shadow-md shrink-0 ${selectedBuyer.avatarBg} ${selectedBuyer.avatarText}`}
          >
            {selectedBuyer.initials}
          </div>
          <div className="flex-1 space-y-3">
            <div>
              <h2 className="text-3xl font-extrabold text-gray-900">{selectedBuyer.name}</h2>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-blue-600 font-medium">{selectedBuyer.handle}</span>
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600 pt-2">
              <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                ...
                <span className="font-medium">{selectedBuyer.raw?.email || "No Email provided"}</span>
              </div>
              <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                <Phone size={16} className="text-gray-400" />
                <span className="font-medium">{selectedBuyer.phone || "No Phone provided"}</span>
              </div>
              <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                <Calendar size={16} className="text-gray-400" />
                <span className="font-medium">Joined {selectedBuyer.joinedDate || "N/A"}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-5 flex flex-col justify-between">
            ...
            <h4 className="text-3xl font-extrabold text-blue-900">{selectedBuyer.totalOrders}</h4>
          </div>
          <div className="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-xl p-5 flex flex-col justify-between">
            ...
            <h4 className="text-3xl font-extrabold text-green-900">{selectedBuyer.totalSpent || "$0.00"}</h4>
          </div>
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-xl p-5 flex flex-col justify-between">
            ...
            <h4 className="text-3xl font-extrabold text-purple-900 capitalize">{selectedBuyer.raw?.role || 'User'}</h4>
          </div>
          <div className="bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200 rounded-xl p-5 flex flex-col justify-between">
            ...
            <h4 className="text-lg font-bold text-orange-900 mt-2">{selectedBuyer.raw?.is_verified ? 'Verified' : 'Unverified'}</h4>
          </div>
        </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
              ...
              {hasAddress ? (
                <div className="space-y-2 text-gray-700">
                  <p className="font-medium text-gray-900 whitespace-pre-wrap">{selectedBuyer.raw?.address}</p>
                </div>
              ) : (
              ...
              <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                    ...
                    <span className="font-semibold text-gray-900 text-base">{selectedBuyer.raw?.business_name || selectedBuyer.raw?.seller_details?.business_name || 'N/A'}</span>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                    <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">GST Number</span>
                    <span className="font-semibold text-gray-900 text-base">{selectedBuyer.raw?.gst_number || selectedBuyer.raw?.seller_details?.gst_number || 'N/A'}</span>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                    <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">Total Listings</span>
                    <span className="font-semibold text-gray-900 text-base">{selectedBuyer.raw?.seller_details?.total_listings || 0}</span>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                    <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">Seller Profile ID</span>
                    <span className="font-semibold text-gray-900 text-base">{selectedBuyer.raw?.seller_profile_id || 'N/A'}</span>
                  </div>
                </div>
              </div>
            )}
             ...
             <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
                ...
                <div className="space-y-3">
                  {selectedBuyer.status !== "Rejected" ? (
                    <button
                      type="button"
                      onClick={() => updateBuyerStatus(selectedBuyer.id, 'Rejected')}
                        ...
                        <div className="flex items-center gap-2 relative">
                          ...
                          {/* MENU */}
                            ...
                            <button
                                ...
                                className="p-1 px-1.5 text-blue-500 text-sm border border-blue-100 bg-blue-50 hover:bg-blue-100 transition flex items-center gap-1 ml-2"
                            >
                                <Eye size={14} /> View
                            </button>
                          )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className={`w-6 h-6 flex items-center justify-center border border-gray-300 ${currentPage === 1
                ...
                (page, index) =>
                  page === "..." ? (
                    ...
                    <button
                      ...
                      className={`w-6 h-6 flex items-center justify-center text-xs font-medium ${currentPage === page
              ...
              <button
                ...
                className={`w-6 h-6 flex items-center justify-center border border-gray-300 ${currentPage === totalPages ||
                    totalPages === 0
                    ? "text-gray-300 cursor-not-allowed"
                    : "text-gray-600 hover:bg-gray-50 cursor-pointer"
                  }`}
